...
def add_patient():
  ...
  # Log pour débogage
        ...
        for patient in patients:
            ...
            existing_patient = patients_collection.find_one({"patient_id": patient_data["patient_id"],"DoctorId":patient_data["DoctorId"]})
            if existing_patient:
                # Merge images
...
@patients_bp.route("/delete-patient/<doctorId>", methods=["DELETE"])
def delete_many_patient(doctorId):
    try:
...
@patients_bp.route("/allnormalize/<DoctorId>", methods=["GET"])
def get_all_patients_normalize(DoctorId):
    try:
